...
export default function Currency() {
  ...
  return (
    <Screen>
      ...
      <h2 style={{ color: C.navy, fontSize: 19, fontWeight: 800, marginTop: 10 }}>Choose your currency</h2>
      <p style={{ color: C.textMuted, fontSize: 13, marginTop: 4 }}>Select the currency you want to use in the app.</p>
      <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 20 }}>
        {currencies.map((c) => (
          <label key={c} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 4px", cursor: "pointer" }}>
            <input type="radio" name="cur" checked={currency === c} onChange={() => setCurrency(c)} style={{ width: 18, height: 18, accentColor: C.green }} />
            <span style={{ fontSize: 14.5, fontWeight: 700, color: C.navy }}>{c}</span>
          </label>
        ))}
      </div>
      <div style={{ flex: 1 }} />
      <PrimaryButton onClick={() => navigate("budgetHub")}>Continue</PrimaryButton>
    </Screen>
  );
}
