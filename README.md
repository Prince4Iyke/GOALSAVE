...
- No backend — all data (transactions, goals, notifications, budget) lives
  in memory via React state and resets on page reload. That's intentional
  for an MVP demo; swapping in a real API mainly means replacing what's
...
- No backend/persistence — refreshing the page resets all data
- Google/Apple sign-in, Bank Accounts, Preferences, Change Password,
  ...
  they're UI-complete but not wired to real services
